...
const AdminDashboard = () => {
    ...
    filteredOrders.forEach(order => {
      (order.items || []).forEach(item => {
        rows.push({
          ...
          "Advance Amount": order.advance || 0
        });
      });
    });
  ...
  const exportToPDF = () => {
    try {
      ...
      const tableColumn = ["Date", "Phone", "Item Name", "Quantity", "Distributor", "Advance Amount"];
      const tableRows = [];

      formatOrdersForExport().forEach(row => {
        tableRows.push([
          row.Date,
          row.Phone,
          row["Item Name"],
      ...
      autoTable(doc, {
        ...
        columnStyles: {
          ...
          3: { cellWidth: 20 }, // Qty
          4: { cellWidth: 45 }, // Distributor
          5: { cellWidth: 30 }  // Advance
        }
      });
